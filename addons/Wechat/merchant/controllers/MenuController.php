...
class MenuController extends BaseController
{
    ...
    {
        $type = Yii::$app->request->get('type', MenuTypeEnum::CUSTOM);
        $data = Menu::find()
            ->where(['type' => $type])
        ...
        // 查询下菜单
        !$models && Yii::$app->services->base->getWechatError(Yii::$app->wechat->app->menu->current());

        return $this->render('index', [
            'pages' => $pages,
            'models' => $models,
            'type' => $type,
            'types' => MenuTypeEnum::getMap(),
        ]);
    }
    ...
    public function actionSave($id)
    {
        if ($id) {
            ...
            // 解析微信接口是否报错
            if ($error = Yii::$app->services->base->getWechatError($createReturn, false)) {
