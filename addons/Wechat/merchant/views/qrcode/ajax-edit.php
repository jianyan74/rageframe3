...
<div class="modal-body">
    ...
    <div class="row <?= !$model->isNewRecord ? 'hide' : '' ?>">
        <div class="col-md-4">
            <?= $form->field($model, 'model_type')->dropDownList(QrcodeModelTypeEnum::getMap(), ['onclick' => []]) ?>
        </div>
        <div class="col-md-8">
            <div id="model1">
    ...
    <div class="form-group">
        <div class="hint-block">目前有2种类型的二维码, 分别是临时二维码和永久二维码, 前者有过期时间, 最大为30天（2592000秒）, 但能够生成较多数量, 后者无过期时间,
            数量较少(目前参数只支持1--100000).
        </div>
    </div>
...
<script>
    $("select[name='Qrcode[model_type]']").change(function () {
        var showId = $(this).val();
        var hideId = parseInt(showId) === 1 ? 2 : 1;
        $('#model' + hideId).hide();
        $('#model' + showId).show();
    })
</script>