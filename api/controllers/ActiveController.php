...
use common\behaviors\TrafficShaperBehavior;
...
class ActiveController extends \yii\rest\ActiveController
{
     ...
     * @var int
     */
    protected $memberType = MemberTypeEnum::MEMBER;
    ...
    public function behaviors()
    {
        ...
        if (Yii::$app->params['trafficShaperValidity'] == true) {
            // 令牌桶限流
            $behaviors['trafficShaper'] = [
                'class' => TrafficShaperBehavior::class,
                'enable' => true,
            ];
        }
    ...
    public function beforeAction($action)
    {
        ...
        if (
            ...
            $this->memberType !== Yii::$app->user->identity->member_type
        ) {
        ...
        // 判断 IP 黑名单
        $blacklistOpen = Yii::$app->services->config->backendConfig('sys_ip_blacklist_open');
        if ($blacklistOpen == StatusEnum::ENABLED && !empty($blacklistIp = Yii::$app->services->config->backendConfig('sys_ip_blacklist'))) {
            $ipList = StringHelper::parseAttr($blacklistIp);
            if (ArrayHelper::ipInArray(Yii::$app->request->userIP, $ipList)) {
                // 记录行为日志
                Yii::$app->services->actionLog->create('visit', '限制IP访问', 0, [], false);

                throw new UnauthorizedHttpException('限制访问');
            }
        }
