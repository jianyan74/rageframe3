...
class AliPay
{
    ...
    public function transferQuery(array $info)
    {
        ...
        return $data['alipay_fund_trans_order_query_response'] ?? '';
    }
     ...
     * @throws InvalidRequestException
     */
    public function settle(array $info)
    {
        $gateway = $this->create();
        $request = $gateway->settle();
        $response = $request->setBizContent($info)->send();
     ...
     * @throws InvalidRequestException
     */
    public function capture(array $info)
    {
        ...
        try {
            ...
            if ($response->isPaid()) {
                ...
                throw new UnprocessableEntityHttpException('收款失败');
            }
        } catch (Exception $e) {
