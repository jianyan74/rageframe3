...
class Files extends InputWidget
{
    ...
    public function init()
    {
        ...
        $uploadUrl = [
            'images' => Url::to(['/files/images']),
            'videos' => Url::to(['/files/videos']),
            'voices' => Url::to(['/files/voices']),
            'files' => Url::to(['/files/files']),
        ];

        // 默认配置信息
        $this->typeConfig = Yii::$app->params['uploadConfig'][$this->type];
        // 切换上传驱动
        if ($storageDefault = Yii::$app->services->config->backendConfig('storage_default')) {
        ...
        $this->config = ArrayHelper::merge([
            ...
            'mergeUrl' => Url::to(['/files/merge']),
            'getOssPathUrl' => Url::to(['/files/get-oss-path']),
            'verifyMd5Url' => Url::to(['/files/verify-md5']),
    ...
    public function run()
    {
        ...
        // 阿里云直传
        if (AttachmentDriveEnum::OSS_DIRECT_PASSING == $this->config['formData']['drive']) {
            $path = $this->typeConfig['path'] . date($this->typeConfig['subName'], time()) . "/";
            $oss = Yii::$app->services->extendUpload->ossConfig($this->config['fileSingleSizeLimit'], $path, 60 * 60 * 2, $this->type);
