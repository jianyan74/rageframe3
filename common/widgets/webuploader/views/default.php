                ...
                <li class="upload-box <?php if(!empty($value) && $config['pick']['multiple'] == false){?>hide<?php } ?>">
                    ...
                    <?php if ($themeConfig['select'] === true) {?>
                        <div class="upload-box-bg hide befor-upload">
                            <a class="first openUploadIframe" href="<?= Url::to(['/files/selector', 'box_id' => $boxId, 'upload_type' => $type, 'multiple' => $config['pick']['multiple'], 'upload_drive' => $config['formData']['drive']])?>">选择文件</a>
