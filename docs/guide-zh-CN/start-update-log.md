...
> 3.1.x 开始每次升级代码后建议再去 系统->应用管理->系统更新->数据库升级  
...
- 增加: 复制 InputWidget
- 增加: 会员 Excel 导入
- 增加: 会员余额/积分/成长值/消费日志的筛选导出
- 增加: ZipHelper 辅助类
- 优化：bootstrapTable 的内容显示方式
- 优化: 图片裁剪组件支持上传到第三方存储 (fengyingxuetang)
- 修复: 无限极分类查询所有子分类个别数据会查询数据错误
- 修复: ExcelHelper 类导出如果是空数据会异常
- 修复: 微信支付个别金额计算会出现最终金额错误
- 修复: 微信退款报错
...
- 增加: 微信小程序插件
- 修复: 用户数据统计错误
- 修复: linkage 小组件一个页面多次调用时 ajax 请求会被覆盖
- 优化: 无商户判断显示
- 兼容: PHP 8.x
...
- 修复: 3.1.29版本伪删除行为影响系统默认删除

### v3.1.29
updated 2023.7.10

- 增加: 消息提醒辅助说明
- 增加: 记录图片文件资源入表判断, 避免接口的冗余资源干扰后台使用
- 增加: 标签行为, 减少冗余代码
- 增加: 会员注销
- 增加: 会员正常/注销/黑名单状态显示
- 修复: 会员发票字段缺失
