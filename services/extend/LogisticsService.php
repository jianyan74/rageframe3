 ...
 * 物流查询
 *
 * $order = Yii::$app->-service->extendLogistics->aliyun();
 *
 * $order->getCode(); // 状态码
 * $order->getMsg(); // 状态信息
...
class LogisticsService extends Service
{
    ...
    public function init()
    {
        ...
        if (Yii::$app->services->devPattern->isSAAS()) {
            $defaultConfig = Yii::$app->services->config->merchantConfigAll();
        } else {
            $defaultConfig = Yii::$app->services->config->backendConfigAll();
        }
    ...
    protected function logistics($provider)
    {
        ...
        return new Logistics($this->config);
    }
}
