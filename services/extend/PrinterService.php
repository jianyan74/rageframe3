...
class PrinterService
{
    ...
    public function receiptPrinter(Config $config, $data)
    {
        ...
        switch ($config->name) {
            case ExtendConfigNameEnum::YI_LIAN_YUN :
                Yii::$app->services->extendYiLianYun->text($data, '', $model);
                break;
            case ExtendConfigNameEnum::FEI_E :
                Yii::$app->services->extendFeiEYun->print($data, $model);
                break;
            case ExtendConfigNameEnum::XP_YUN :
                Yii::$app->services->extendXpYun->print($data, $model);
                break;
        }
    ...
    {
        return Yii::$app->services->extendConfig->findByType(ExtendConfigTypeEnum::RECEIPT_PRINTER, $merchant_id, StatusEnum::ENABLED);
    }
}