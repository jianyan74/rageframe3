 ...
 * 个推
 *
 *     Yii::$app->services->extendGeTui->client()->ios($title, $content, $clientId, $transmissionContent = [])
 *
 * Class GeTuiService
...
class GeTuiService extends Service
{
    ...
    public function client($config = [])
    {
        $config = ArrayHelper::merge([
            'app_key' => Yii::$app->services->config->backendConfig('getui_app_id'),
            'app_id' => Yii::$app->services->config->backendConfig('getui_app_key'),
            'master_secret' => Yii::$app->services->config->backendConfig('getui_master_secret'),
            'logo_url' => Yii::$app->services->config->backendConfig('getui_logo_url')
        ], $config);
     ...
     * @param $content
     * @param $clientId
     * @param array $transmissionContent 透传内容
     */
