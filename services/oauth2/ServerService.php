...
class ServerService extends Service
{
    ...
    {
        $privateKey = 'file://' . Yii::getAlias(Yii::$app->services->config->backendConfig('oauth2_rsa_private'));

        // 如果私钥文件有密码
        if (!empty(Yii::$app->services->config->backendConfig('oauth2_rsa_private_encryption'))) {
            $privateKey = new CryptKey(
                $privateKey,
                Yii::$app->services->config->backendConfig('oauth2_rsa_private_password'),
    ...
    {
        $encryptionKey = Yii::$app->services->config->backendConfig('oauth2_encryption_key'); // 加密密钥字符串
