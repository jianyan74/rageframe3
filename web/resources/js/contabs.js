$(function () {
    ...
    function scrollToTab(element) {
        ...
        //实际滚动宽度
        var scrollVal = 0;
        var rightOuterWidth = $(element).next().outerWidth(true);
        if (rightOuterWidth === 'undefined' || rightOuterWidth === undefined) {
            rightOuterWidth = 0;
        }

        if ($(".page-tabs-content").outerWidth() < visibleWidth) {
            scrollVal = 0;
        } else if (marginRightVal <= (visibleWidth - $(element).outerWidth(true) - rightOuterWidth)) {
            if ((visibleWidth - rightOuterWidth) > marginRightVal) {
    ...
    function menuItem() {
        ...
        var dataUrl = $(this).attr('href'),
            ...
            menuTitle = $.trim($(this).data('title')),
            menuName = menuTitle.length > 0 ? menuTitle : $.trim($(this).text()),
            flag = true;
        ...
        // 选项卡菜单不存在
        if (flag) {
            var str = '<a href="javascript:;" class="active J_menuTab" data-id="' + dataUrl + '">' + menuName + '<i class="iconfont iconguanbi"></i></a>';
